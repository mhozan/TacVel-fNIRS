function data = loadDirectory_( rootFolder, folderHierarchy, loadFunc, fileExt )
%call loadNIRX2 function, which loads ADI .adicht data as auxillary.
if nargin < 4,
...
%            disp(['loading: ' files(iFile).name]);
        ...
        if ~isempty(tmp)
            if(~isempty(strfind(func2str(loadFunc{i}),'nirs.cnl.loadNIRx2')) && ...
