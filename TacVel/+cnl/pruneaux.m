                     ...
                     for vv=1:length(keysaux_pruned)
                         if valuesaux_pruned{vv}.Fs == Fs_resample
                            ...
                            % Fs_i = valuesaux_pruned{vv}.Fs;
                             ...
                             if tvec_resampled(end)<data(i).time(end) %this happens only for '20221121A' and we pad the aux with extra 0s at the end to avoid errors
                                 % tttvec_resampled =[tvec_resampled
                                 padwithzero= zeros(round((-tvec_resampled(end)+data(i).time(end))*valuesaux_pruned{vv}.Fs),1);
                                 tvecpad = tvec_resampled(end)+(1/valuesaux_pruned{vv}.Fs)...
                                     :(1/valuesaux_pruned{vv}.Fs):data(i).time(end);
                                 data_resampled = [data_resampled;padwithzero];
                                 tvec_resampled = [tvec_resampled;tvecpad.'];
                             end
                             ...
                             while length(data_resampled)~=size(data(i).data,1)
                                 if abs(length(data_resampled)-size(data(i).data,1))>10
                                    % disp(mfilename)
                                    keyboard
                                    % warning('Too much diparity between the lengths of resampled_aux data and fnirs data')
                                 end
