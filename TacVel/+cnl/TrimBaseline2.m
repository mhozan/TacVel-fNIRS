classdef TrimBaseline2 < nirs.modules.AbstractModule
%% TrimBaseline - Removes excessive baseline at the beginning or end of file.
%     %modified by Mohsen Hozan 20230530 to handle different sampling freq
...
%     resetTime    - flag indicating whether the time vector should be reset so t(1)=0 (default: false)
    ...
    properties
        preBaseline  = 90;  % maximum baseline (seconds) at the beginning of scan
        postBaseline = 60;  % maximum baseline (seconds) after final task period ends
            ...
            for i = 1:numel(data)
                ...
                if(obj.Trim_Auxillary_Data && isa(data(i),'nirs.core.Data') && data(i).auxillary.count>0)                    
                    for j=1:data(i).auxillary.count
                        ...
                        lst_aux = dd.time >= t_min & dd.time <= t_max; %take care of different sampling frequency
                        dd.data=dd.data(lst_aux,:);
                        dd.time=dd.time(lst_aux);
