classdef pruneaux_backup < nirs.modules.AbstractModule
        ...
        function data = runThis( obj, data )
            ...
            for i = 1:numel(data)
                ...
                if ~isempty(acclindices)
                    ...
                    fl=10;
                    fh=0.5;
                    Fs = combinedaccelerometer{1}.Fs;
                    x=combinedaccelerometer{1}.data(:);
                    x=clipAndNormalizeOutliers(x,'lowerprctile',lowerprctile,'multiplyfactor',multiplyfactor);
                    y=combinedaccelerometer{2}.data(:);
                    y=clipAndNormalizeOutliers(y,'lowerprctile',lowerprctile,'multiplyfactor',multiplyfactor);
                    z=combinedaccelerometer{3}.data(:);
                    z=clipAndNormalizeOutliers(z,'lowerprctile',lowerprctile,'multiplyfactor',multiplyfactor);
                    % XplusYplusZ=clipAndNormalizeOutliers(x+y+z);


                    x_f = applyBandPassFilter(x, 'lowpass', fl, 'highpass', fh, 'Fs', Fs);
                    y_f = applyBandPassFilter(y, 'lowpass', fl, 'highpass', fh, 'Fs', Fs);
                    z_f = applyBandPassFilter(z, 'lowpass', fl, 'highpass', fh, 'Fs', Fs);
                    % xyz_filtered = applyBandPassFilter(x.*y.*z, 'lowpass', fl, 'highpass', fh, 'Fs', Fs);;
                    % XFpYFpZF = applyBandPassFilter(x+y+z, 'lowpass', fl, 'highpass', fh, 'Fs', Fs);;
                    % XplusYplusZ_filtered = applyBandPassFilter(XplusYplusZ, 'lowpass', fl, 'highpass', fh, 'Fs', Fs,'filter_order',fir1order);
                    % XplusYplusZ_filtered = applyBandPassFilter(x_c+y_c+z_c, 'lowpass', fl, 'highpass', fh, 'Fs', Fs,'filter_order',fir1order);
                    XplusYplusZ_filtered = applyBandPassFilter(x+y+z, 'lowpass', fl, 'highpass', fh, 'Fs', Fs,'filter_order',fir1order);
                    % if any(isnan(XplusYplusZ_filtered)) || any(isinf(XplusYplusZ_filtered)) % || ~isnumeric(xyzaccl) || isempty(xyzaccl)
                    %     keyboard
                    % end
                    winlen = round(.1*Fs);
                    % [yupper,~] = envelope(xyzaccl,winlen,'analytic');
                    [xyz_acc_env,~] = envelope(XplusYplusZ_filtered,winlen,'rms'); %rms envelope
                    % [xyz_acc_env] = movingavg_abs_emg(XplusYplusZ_filtered);
                    ...
                    ax(1)=subplot(2,1,1);
                    l1 = plot(combinedaccelerometer{1}.data); l1.DisplayName = 'x_acc'; hold on
                    l2 = plot(combinedaccelerometer{2}.data); l2.DisplayName = 'y_acc';
                    l3 = plot(combinedaccelerometer{3}.data); l3.DisplayName = 'z_acc';
                    legend('Interpreter','none')

                    ax(2)=subplot(2,1,2);
                    l4 = plot(XplusYplusZ_filtered); l4.DisplayName = 'x+y+z_acc'; hold on;
                    l5 = plot(xyz_acc_env-2,'r','LineWidth',2); l5.DisplayName = 'upper_env_analytic';
                    legend('Interpreter','none')


                    xyz = [x_f,y_f,z_f];
                    % values_aux{end}.data = xyz; %replace its data
                    values_aux{end}.data = XplusYplusZ_filtered;
                ...
                % clip_threshold = 99;
                 ...
                 emghand = applyBandPassFilter(emghand, 'lowpass', 0.5, 'highpass', 10, 'Fs', values_aux{emghandindex}.Fs);
                 ...
                 micdata = applyBandPassFilter(micdata, 'lowpass', 0.5, 'highpass', 10, 'Fs', values_aux{micindex}.Fs);
                 ...
                 ax(vv+1)=subplot(N_aux+1,1,vv+1);
