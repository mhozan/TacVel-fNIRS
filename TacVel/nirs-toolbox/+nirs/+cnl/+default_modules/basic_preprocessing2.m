...
jobs = nirs.modules.ExportData(jobs);
jobs.Output = 'raw_pruned_trimmed_TDDR';
...
% jobs = nirs.modules.BaselinePCAFilter(jobs);
% jobs = nirs.modules.ExportData(jobs);
% jobs.Output = 'test2_raw_pruned_resampled_trimmed';
% jobs = advanced.nirs.modules.ShortSeperationCCAFilter(jobs);


% jobs = nirs.modules.LabelShortSeperation(jobs);
% jobs.max_distance = 10;
...
% jobs = advanced.nirs.modules.ShortDistanceFilter(jobs);
% jobs = nirs.modules.RemoveShortSeperations(jobs);  %%%%
% jobs = nirs.modules.RemoveShortSeperations(jobs);  %%%%
